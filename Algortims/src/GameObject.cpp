...
GameObject::GameObject(const std::string& filePath, ID3D11Device* pDevice, ID3D11DeviceContext* pContext, LPCWSTR vertexDir, LPCWSTR pixelDir, float depthZ, float a)

{
	player = std::make_unique<Player>(filePath, pDevice, pContext, vertexDir, pixelDir, depthZ, a);
}
...
void GameObject::Bind(ID3D11DeviceContext* pContext, ID3D11Device* pDevice, const wchar_t* fileName, XMMATRIX matrix)
{
	player->Bind(pContext, pDevice, fileName, matrix);
}

void GameObject::Draw(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	player->Draw(pContext, pDevice);
}
